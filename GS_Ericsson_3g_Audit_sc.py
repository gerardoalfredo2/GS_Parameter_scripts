def GS_Audit_Ericsson3g(par_file, input_folder):
    ...
    cellsites = pandas.read_excel(open(par_file, 'rb'), sheetname='3gCells')
                ...
                if str(GS_value[0]) == '-':
                    ...
                    for ind in complains.index:
                        ...
                        if str(complains.loc[ind, 'Id']) == str(complains.loc[ind, 'RNC_name']):
                            ...
                            variab= str(list(str(complains.loc[ind,'Id']).split('-'))[0])[:-1]
                            complains.loc[ind,'Cell_name'] = variab
                        elif '_' in list(complains.loc[ind, 'Id']):
                            complains.loc[ind, 'Cell_name'] = str(complains.loc[ind, 'Id_2'])[:-1]
                        elif str(complains.loc[ind, 'Id']) == '1':
                            complains.loc[ind, 'Cell_name'] = str(complains.loc[ind, 'Id_2'])[:-1]
                        else:
                            complains.loc[ind, 'Cell_name'] = str(complains.loc[ind, 'Id'])[:-1]
                        print 'index: ', ind
